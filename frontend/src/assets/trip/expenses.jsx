                ...
                <li key={expense.id} className={`${styles.expenseItem} bg-gray-50 p-4 rounded-xl flex items-center justify-between transition-all hover:shadow-md`}>
                    ...
                    <div className="flex items-center space-x-4">
                        ...
                        <button className="delete-btn text-gray-400 hover:text-red-500 transition-all" data-id="${expense.id}">
                            <i className="fa-solid fa-xmark text-xl" onClick={() => props.setVisibleDeleteModal({ state: true, id: expense.id, name: expense.description })}></i>
                        </button>
                    </div>
                </li>
...
export function ExpensesPage() {
    const { currentTripId } = useTripInfo();
    ...
    async function fetchAndRenderData() {
        try {
            ...
            const initialBudget = parseFloat(budgetData.currentTripInfo.budget);

            // setStartingBudget(initialBudget);
            const totalExpenses = (parseFloat(budgetData.totalExpenses)).toFixed(2);
    ...
    return (<>
                    ...
                    <div className={`${styles.categoryCardsContainer} ${expensesFadeOut ? styles.fadeOut : ''}`}>
                        <section>
                            ...
                            <ul id="expense-list" className="space-y-4">
                                ...
                                {console.log('hello')}
                                <RenderExpensesPage expensesDemo={expensesDemo} setVisibleDeleteModal={setVisibleDeleteModal} />
                            </ul>
                        </section>
                    </div>
                }
        ...
        {visibleDeleteModal.state && <DeleteExpense setVisibleDeleteModal={setVisibleDeleteModal} visibleDeleteModal={visibleDeleteModal} setExpensesDemo={setExpensesDemo} expensesDemo={expensesDemo} />}

...
export function NewTrip() {
    ...
    const {  setTrip_name , setCurrentTripId} = useTripInfo();
   ...
   useEffect(() => {
    ...
    const handleClick = (e) => {
        if (!destinationInputRef.current?.contains(e.target)) {
            setSuggestions([]);
        }
    };
    ...
    return (<>
        ...
        <div id="new-trip-modal" className={styles.fullPageModal}>
            <Link to="/all_trips" id="back-to-app-btn" className="absolute  text-black text-lg z-10 hover:text-gray-500 transition-colors"
            style={{top: '5vh', left: '9vw'}}
            >
            ...
            <div className={`${styles.fullPageContent} `}>
                ...
                <form id="new-trip-form" className={`${styles.form} space-y-6 text-left`} onSubmit={handleNewTripSubmit}>
                    ...
                    <div className={`${styles.inputGroup} ${styles.inputGroupSuggestions}`}>
                        <input type="text" id="new-trip-destination" onChange={handleInputChange} onFocus={handleInputChange} name="destination" required value={destination} placeholder=" " autoComplete='off'
                            className="w-full px-2 py-2 text-white rounded-lg focus:outline-none transition-all" ref={destinationInputRef} />
                        <Suggestions />
